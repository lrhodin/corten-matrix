...
// bridgeFiltered mirrors IMConfig.BridgeFilteredChats. When false (the
// default) portals whose cloud_chat rows are all iCloud-filtered — Apple's
...
func (s *cloudBackfillStore) scrubBatchIfEligible(ctx context.Context, cutoff int64, bridgeID string, bridged *bridgedIDSet, candidates []cloudScrubCandidate) (int64, error) {
	...
	result, err := s.db.Exec(ctx, `
		...
		WHERE login_id=$1
		  ...
		  AND (tapback_type IS NULL OR tapback_type < 2000)
		  AND updated_ts < $2
		  AND (deleted=TRUE OR (`+pendingGate+`))
